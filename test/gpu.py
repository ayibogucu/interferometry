...
fft_centered_image = cp.roll(
    cp.roll(fft_masked_image, shift_y, axis=0), shift_x, axis=1
)
...
ax[0, 1].imshow(cp.log(cp.abs(fft_image) + 1), cmap="gray")
ax[0, 1].set_title("FFT Magnitude (Original)")
ax[0, 1].axis("off")

# Display the masked FFT magnitude
ax[0, 2].imshow(cp.log(cp.abs(fft_masked_image) + 1), cmap="gray")
ax[0, 2].set_title("Masked FFT Magnitude")
ax[0, 2].axis("off")

# Display the center-shifted FFT magnitude
ax[1, 0].imshow(cp.log(cp.abs(fft_centered_image) + 1), cmap="gray")
