...
def main():
    ...
    for idx in range(0, len(all_files), BATCH_SIZE):
        ...
        aux.plot_batch(array_batch, "this is array_batch")

        ## PHASE ALGORITHM
        phase_batch = phase.fft_batch(array_batch, MASK_RADIUS)
        aux.plot_batch(phase_batch, "this is phase_batch")

        ## UNWRAP
        unwrapped_phase_batch = unwrap.quality_guided(phase_batch=phase_batch)
        aux.plot_batch(unwrapped_phase_batch, "this is unwrapped_phase_batch")

        ##ODP
        height_map_batch = opd.OPD(unwrapped_phase_batch, WAVELENGTH)
        aux.plot_batch(height_map_batch, "this is height_map_batch", auto_close=True)
